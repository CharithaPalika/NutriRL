...
class DDQNAgent(BaseAgent):
    ...
    Q-values are produced by a single forward pass over the menu: QNetwork
    takes (physiological_state, food_embeddings) and returns one Q-value per
    menu slot plus one for "skip" -- shape (B, num_foods + 1). This replaces
    the old per-fixed-action one-hot loop, since with a resampled menu each
    step, "action index 2" doesn't have a stable identity across steps; the
    food sitting in slot 2 right now is what's stable, and that's exactly
    what the food_embeddings tensor encodes.
    """
    ...
    def __init__(self, env, device="cpu", **kwargs):
        ...
        try:
            ...
            self.food_dim = env.observation_space["food_embeddings"].shape[-1]
            self.act_dim = env.action_space.n  # num_foods + 1
    ...
    def act(self, obs, deterministic=False):
        ...
        food = torch.tensor(
            obs["food_embeddings"], dtype=torch.float32, device=self.device
        ).unsqueeze(0)
        ...
        if (not deterministic) and np.random.rand() < self.epsilon:
            ...
            with torch.no_grad():
                q_vals = self.q_net(phys, food).squeeze(0)  # (num_foods + 1,)
    ...
    def _update(self):
        ...
        food = torch.tensor(
            [o["food_embeddings"] for o in obs],
            dtype=torch.float32,
            device=self.device,
        )
        ...
        next_food = torch.tensor(
            [o["food_embeddings"] for o in next_obs],
            dtype=torch.float32,
            device=self.device,
        )
        ...
        with torch.no_grad():
            # Double DQN: select next action with the online net, evaluate
            # with the target net -- both now in one forward pass each.
            q_next_online = self.q_net(next_phys, next_food)        # (B, K+1)
            next_actions = q_next_online.argmax(dim=1)               # (B,)

            q_next_target = self.target_q_net(next_phys, next_food)  # (B, K+1)
        ...
        q_all = self.q_net(phys, food)  # (B, K+1)
        q = q_all.gather(1, act_idx.unsqueeze(1)).squeeze(1)
    ...
    def train(self, log_wandb=False, printing=True, **kwargs):
        ...
        for ep in tqdm(range(train_args["num_episodes"])):
            ...
            while not done:
                ...
                curr_obs = next_obs
                ep_returns += reward
                ep_eats += int(action != self.skip_action)
                if (
